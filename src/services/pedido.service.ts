...
class PedidoService {
        ...
        carrito.articulos.map(async (articulo: any) => {
          ...
          if (articuloNoInsumo?.esInsumo) {
            ...
            if (!cumpleValidacion) {
              return {
                mensaje:
                  'No hay stock suficiente para poder realizar este articulo, por favor ingrese otro al carrito.',
                status: 400,
              };
            }
            ...
            detallePedido.cantidad = articulo.cantidad;
            detallePedido.subtotal = articuloManufacturado.precioVenta * articulo.cantidad;
            // Antes de guardar en la base de datos, ya vamos calculando el total del pedido en base a los articulos que vamos agregando.
            nuevoPedido.total = nuevoPedido.total = detallePedido.subtotal;
  ...
  async verificaciones(articuloManufacturado: ArticuloManufacturado): Promise<boolean> {
    const articulosInsumo = await getRepository(ArticuloInsumo).find({ where: { articuloManufacturado } });
    let tieneStock = true;
    await Promise.all(
